...
interface FastboatPayload {
  bookingType?: "fastboat";
  bookingCode: string;
  passenger: Passenger;
  schedule: {
    operator: string;
...
type Payload = FastboatPayload | RentalPayload;
...
function renderFastboatEmail(b: FastboatPayload) {
...
<body style="font-family:Arial,sans-serif;color:#0c4a6e;background:#f0f9ff;padding:24px;margin:0;">
  <div style="max-width:600px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;border:1px solid #e0f2fe;">
    <div style="background:linear-gradient(135deg,#0c4a6e,#0369a1);color:#fff;padding:20px 24px;">
      <p style="margin:0;font-size:11px;letter-spacing:2px;opacity:.8;">NEW FASTBOAT BOOKING — tapToGo</p>
    ...
    <table style="width:100%;border-collapse:collapse;padding:24px;">
      ...
      <tr><td style="padding:0 24px 16px;font-size:14px;color:#334155;">${b.schedule.operator} (${b.schedule.boatType})</td></tr>
      ...
      <tr><td style="padding:0 24px 8px;font-size:14px;color:#334155;">Ticket: ${fmtRupiah(b.ticketTotal)}<br>Harbour Tax: ${taxStatus}</td></tr>
      <tr><td style="padding:8px 24px 24px;font-size:20px;color:#0369a1;font-weight:800;">Total: ${fmtRupiah(b.grandTotal)}</td></tr>
...
function renderRentalEmail(b: RentalPayload) {
  return `<!doctype html>
<html><head><meta charset="utf-8"><title>New Rental ${b.bookingCode}</title></head>
<body style="font-family:Arial,sans-serif;color:#0c4a6e;background:#f0f9ff;padding:24px;margin:0;">
  <div style="max-width:600px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;border:1px solid #e0f2fe;">
    <div style="background:linear-gradient(135deg,#0c4a6e,#0369a1);color:#fff;padding:20px 24px;">
      <p style="margin:0;font-size:11px;letter-spacing:2px;opacity:.8;">NEW RENTAL BOOKING — tapToGo</p>
      <h1 style="margin:4px 0 0;font-size:22px;">${b.bookingCode}</h1>
    </div>
    <table style="width:100%;border-collapse:collapse;padding:24px;">
      <tr><td style="padding:16px 24px 8px;font-weight:bold;font-size:13px;color:#0369a1;text-transform:uppercase;letter-spacing:1px;">Vehicle</td></tr>
      <tr><td style="padding:0 24px 16px;font-size:16px;color:#0c4a6e;">
        <strong>${b.rental.vehicle}</strong><br>
        📍 Pickup: ${b.rental.pickupLocation}
      </td></tr>
      <tr><td style="padding:8px 24px;font-weight:bold;font-size:13px;color:#0369a1;text-transform:uppercase;letter-spacing:1px;">Rental Period</td></tr>
      <tr><td style="padding:0 24px 16px;font-size:14px;color:#334155;line-height:1.6;">
        Start: ${fmtDate(b.rental.startDate)}<br>
        Duration: <strong>${b.rental.durationDays} day(s)</strong><br>
        Quantity: <strong>${b.rental.quantity} unit(s)</strong>
      </td></tr>
      <tr><td style="padding:8px 24px;font-weight:bold;font-size:13px;color:#0369a1;text-transform:uppercase;letter-spacing:1px;">Renter</td></tr>
      <tr><td style="padding:0 24px 16px;font-size:14px;color:#334155;line-height:1.6;">
        <strong>${b.passenger.name}</strong>${b.passenger.nationality ? " · " + b.passenger.nationality : ""}<br>
        📧 ${b.passenger.email}<br>
        📞 ${b.passenger.phone}<br>
        🪪 License: ${b.rental.licenseType}
      </td></tr>
      ${b.note ? `<tr><td style="padding:8px 24px;font-weight:bold;font-size:13px;color:#0369a1;text-transform:uppercase;letter-spacing:1px;">Note</td></tr>
      <tr><td style="padding:0 24px 16px;font-size:14px;color:#334155;font-style:italic;">${b.note}</td></tr>` : ""}
      <tr><td style="padding:16px 24px 8px;border-top:1px solid #e0f2fe;font-weight:bold;font-size:13px;color:#0369a1;text-transform:uppercase;letter-spacing:1px;">Payment</td></tr>
      <tr><td style="padding:0 24px 8px;font-size:14px;color:#334155;">
        ${fmtRupiah(b.rental.pricePerDay)} × ${b.rental.durationDays} day × ${b.rental.quantity} unit
      </td></tr>
      <tr><td style="padding:8px 24px 24px;font-size:20px;color:#0369a1;font-weight:800;">Total: ${fmtRupiah(b.grandTotal)}</td></tr>
      <tr><td style="padding:16px 24px;background:#fef3c7;border-top:1px solid #fde047;font-size:13px;color:#854d0e;">
        ⏳ <strong>Status: Pending confirmation</strong><br>
        Confirm availability & deposit with customer via WhatsApp.
      </td></tr>
    </table>
    <div style="background:#f8fafc;padding:12px 24px;text-align:center;font-size:11px;color:#94a3b8;border-top:1px solid #e0f2fe;">
      tapToGo · taptogo.id · ${new Date().toLocaleString("en-GB", { timeZone: "Asia/Makassar" })} WITA
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const isRental = body.bookingType === "rental";
    const subject = isRental
      ? `🛵 New Rental ${body.bookingCode} · ${body.passenger.name} · ${(body as RentalPayload).rental.vehicle}`
      : `🚤 New Booking ${body.bookingCode} · ${body.passenger.name} · ${(body as FastboatPayload).schedule.from} → ${(body as FastboatPayload).schedule.to}`;
    const html = isRental
      ? renderRentalEmail(body as RentalPayload)
      : renderFastboatEmail(body as FastboatPayload);
